...
from core import system_info as sysinfo  # noqa: F401 — re-exported for test patching
from core import window_manager as wm  # noqa: F401 — re-exported for test patching
from core.action_executor import ActionExecutor
from core.action_schemas import validate_action
from core.app_profiles import detect_profile  # noqa: F401 — re-exported for test patching
from core.approval_gate import ApprovalDecision, ApprovalGate
from core.checkpoint import CheckpointManager
from core.engine.failure import FailureTracker
from core.engine.helpers import _clean_messages_for_api, _find_balanced_json_with_key
from core.engine.report import RunReporter
from core.engine.vision import VisionMessageBuilder
...
class AgentEngine:
    ...
    def __init__(
        ...
        self.vision_builder = VisionMessageBuilder(
            config=self.config,
            step_ref=lambda: self.step,
    ...
    def _build_initial_messages(self, goal: str) -> list[dict[str, Any]]:
        ...
        Each sub-step is delegated to a patchable method so tests can
    ...
    def _get_failure_tracker(self) -> FailureTracker:
        """Return the FailureTracker, creating one if necessary.

        Tests construct bare ``AgentEngine`` objects via ``__new__`` that
        skip ``__init__``, so we lazily create a tracker when the
        attribute is missing.
        """
        tracker = getattr(self, "_failure_tracker", None)
        if tracker is None:
            # Ensure _recovery_engine exists for the tracker to use.
            if not hasattr(self, "_recovery_engine"):
                from core.recovery import RecoveryEngine

                self._recovery_engine = RecoveryEngine()
            tracker = FailureTracker(self)
            self._failure_tracker = tracker
        return tracker
    ...
    def _handle_action_failure(
        ...
        Delegates to ``FailureTracker.handle_action_failure``.
        """
        return self._get_failure_tracker().handle_action_failure(
    ...
    def _handle_consecutive_failure(
        ...
        Delegates to ``FailureTracker.handle_consecutive_failure``.
        """
        return self._get_failure_tracker().handle_consecutive_failure(failure_type, messages)
    ...
    # ── Report generation (delegated to RunReporter) ─────────────────

    def _get_reporter(self) -> RunReporter:
        """Return the RunReporter instance, creating one if necessary.

        Tests construct bare ``AgentEngine`` objects via ``__new__`` that
        skip ``__init__``, so we lazily create a reporter when the
        attribute is missing.
        """
    ...
    def _generate_report(self, goal: str, elapsed: float) -> dict[str, Any]:
        ...
        Delegates to ``RunReporter.generate``.
        """
        return self._get_reporter().generate(self, goal, elapsed)

    def _compute_action_counts(self) -> dict[str, int]:
        """Count occurrences of each action type in the forensic log."""
        return self._get_reporter().compute_action_counts(self.forensic_log)
    ...
    def _strip_screenshot_params(params: dict[str, Any] | None) -> dict[str, Any]:
        ...
        return RunReporter.strip_screenshot_params(params)
    ...
    def _build_step_trace(self, entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        return self._get_reporter().build_step_trace(entries)
    ...
    def _build_error_list(self, errors: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        return self._get_reporter().build_error_list(errors)
    ...
    def _build_report_text(
        ...
        return self._get_reporter().build_report_text(
            report, goal, elapsed, success, errors, provider, model, self.step, self.notes
        )
    ...
    # ── Vision / context (delegated to VisionMessageBuilder) ─────────

    def _get_vision_builder(self) -> VisionMessageBuilder:
        """Return the VisionMessageBuilder, creating one if necessary.

        Tests construct bare ``AgentEngine`` objects via ``__new__`` that
        skip ``__init__``, so we lazily create a builder when the
        attribute is missing.
        """
        builder = getattr(self, "vision_builder", None)
        if builder is None:
            builder = VisionMessageBuilder(
                config=getattr(self, "config", {}),
                step_ref=lambda: getattr(self, "step", 0),
                image_history=int(getattr(self, "image_history", DEFAULT_IMAGE_HISTORY)),
            )
            self.vision_builder = builder
        return builder

    def _build_env_context(self) -> str:
        """Gather OS info, active window title, and tenant metadata for the prompt."""
        return self._get_vision_builder().build_env_context()

    def _build_app_context(self) -> str:
        """Build app-profile context for the system prompt."""
        return self._get_vision_builder().build_app_context()
    ...
    def _add_vision_message(self, messages: list[dict[str, Any]], screenshot_b64: str, text: str) -> None:
        ...
        self._get_vision_builder().add_vision_message(messages, screenshot_b64, text)

    def _prune_old_screenshots(self, messages: list[dict[str, Any]]) -> None:
        """Drop the image bytes from older screenshot messages."""
        self._get_vision_builder().prune_old_screenshots(messages)
