...
class MFADetector:
    ...
    MFA_KEYWORDS = {
        'totp': [
            'authenticator', 'verification code', 'verify code', 'auth code',
            'two-factor', '2fa', 'mfa', 'multi-factor', 'totp', 'otp',
            'authenticator app', 'authentication code', 'security code'
        ],
        'sms': [
            'sms', 'text message', 'phone verification', 'mobile code',
            'verify phone', 'phone code', 'sms code', 'text message code',
            'mobile verification', 'phone verification code', 'sms code',
            'verification sms'
        ],
        'email': [
            'email code', 'email verification', 'verify email', 'email token',
            'check your email', 'email otp', 'email verification code',
            'verification email', 'email authentication'
        ],
        'recovery': [
            'recovery code', 'backup code', 'recovery key', 'backup key',
            'recovery phrase', 'restore code'
        ]
    }
    ...
    MFA_DOM_ATTRIBUTES = {
        ...
        'name_pattern': [
            'otp', 'code', 'token', 'mfa', '2fa', 'totp',
            'verification', 'authenticator', 'challenge'
        ],
        'id_pattern': [
            'otp', 'code', 'token', 'mfa', '2fa', 'totp',
    ...
    def _detect_by_structure(self, forms: list[dict], inputs: list[dict],
                            text: str, url: str) -> list[MFAField]:
        ...
        is_login_page = (
            ...
            'sign-in' in url.lower() or
            'auth' in url.lower() or
        ...
        has_login_keywords = any(
            ...
            ['sign in', 'login', 'log in', 'authenticate', 'verify']
        )

        has_mfa_keywords = any(
            kw in text_lower for kw in
            ['verification code', 'authenticator', '2fa', 'mfa', 'otp']
        )
        ...
        for inp in inputs:
            ...
            if (
                ...
                (maxlength and int(maxlength) in [6, 7, 8]) and
                inputmode == 'numeric'
            ):
                confidence = 0.7
                if has_mfa_keywords:
    ...
    def _merge_mfa_fields(self, fields: list[MFAField]) -> list[MFAField]:
        ...
        for field in fields:
            ...
            if key in unique_fields:
                ...
                if type_priority.get(field.input_type, 0) > type_priority.get(existing.input_type, 0):
    ...
    def _calculate_confidence(self, fields: list[MFAField],
    ...
    def _classify_page_type(self, text: str, url: str,
                          forms: list[dict]) -> str | None:
        ...
        if any(kw in text_lower or kw in url_lower for kw in
               ['verification', 'mfa', '2fa', 'authenticator', 'otp']):
            return 'mfa'
