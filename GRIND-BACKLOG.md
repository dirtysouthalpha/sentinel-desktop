...
- [ ] Phase 1: Cut v22.0.0 release tag — push an annotated tag, confirm
  `.github/workflows/release.yml` runs (test → build → publish), verify the PyPI +
  GitHub Release succeeds. Confirm PyPI trusted-publishing is configured (see
  `RELEASING.md`); if not, document the one-time setup as a follow-up.
- [ ] Phase 2: Design the deferred fully-stealth humanization tier — write a spec
  ...
  doc). This is design-only; no code in this phase.
- [ ] Phase 3: Pre-load the Neuralis brain with the sentinel-desktop knowledge base
