...
class RecorderPanel(ctk.CTkFrame):
    ...
    def _on_record(self) -> None:
        ...
        if recorder is None:
            messagebox.showwarning("Recorder", "No recorder instance available."); return
    ...
    def _on_stop(self) -> None:
        ...
        if not script.steps:
            self._set_ready()
            messagebox.showinfo("Recorder", "No actions were recorded."); return
        self._show_save_dialog(script)

    def _show_save_dialog(self, script: Any) -> None:
        dlg = ctk.CTkToplevel(self); dlg.title("Save Recorded Script")
        dlg.geometry("420x320"); dlg.resizable(False, False)
        dlg.transient(self.winfo_toplevel()); dlg.grab_set()
        pad = dict(padx=12, pady=6, sticky="ew"); dlg.grid_columnconfigure(1, weight=1)

        ctk.CTkLabel(dlg, text="Name:", font=("Segoe UI", 12)).grid(row=0, column=0, **pad)
        name_e = ctk.CTkEntry(dlg, font=("Segoe UI", 12)); name_e.insert(0, script.name)
        ...
        desc_e.insert("1.0", script.description); desc_e.grid(row=1, column=1, **pad)
        ...
        def _save():
            ...
            dlg.destroy(); self.status_label.configure(text=f"Saved: {name}")

        ctk.CTkButton(bf, text="💾 Save", width=100, command=_save).pack(side="left", padx=8)
        ctk.CTkButton(bf, text="Cancel", width=100, fg_color="transparent",
    ...
    def _on_play(self) -> None:
        path = filedialog.askopenfilename(
            title="Select Script to Replay", initialdir=_ensure_scripts_dir(),
    ...
    def _show_param_dialog(self, parameters: List[Dict[str, str]]) -> Optional[Dict[str, Any]]:
        dlg = ctk.CTkToplevel(self); dlg.title("Script Parameters")
        ...
        entries: Dict[str, ctk.CTkEntry] = {}; result: Dict[str, Any] = {}; cancelled = [True]
        ...
        for i, p in enumerate(parameters):
            ...
            e.insert(0, p.get("default", "")); e.grid(row=i, column=1, padx=12, pady=6, sticky="ew")
            entries[nm] = e

        br = len(parameters)

        def _ok():
            for n, e in entries.items(): result[n] = e.get()
            cancelled[0] = False; dlg.destroy()

        ctk.CTkButton(dlg, text="OK", width=80, command=_ok).grid(row=br, column=0, padx=8, pady=10)
        ctk.CTkButton(dlg, text="Cancel", width=80, fg_color="transparent",
                       command=dlg.destroy).grid(row=br, column=1, padx=8, pady=10, sticky="w")
        dlg.wait_window()
    ...
    def _run_script(self, path: str, params: Dict[str, Any], script_data: dict) -> None:
        ...
        if engine is None:
            messagebox.showwarning("Playback", "No script engine available."); return
    ...
    def _on_library(self) -> None:
        ...
        dlg = ctk.CTkToplevel(self); dlg.title("📋 Script Library")
        dlg.geometry("560x480"); dlg.transient(self.winfo_toplevel()); dlg.grab_set()
        dlg.grid_rowconfigure(2, weight=1); dlg.grid_columnconfigure(0, weight=1)

        sv = ctk.StringVar()
        ctk.CTkEntry(dlg, placeholder_text="🔍 Search by name or tags…",
                      font=("Segoe UI", 12), textvariable=sv).grid(
        ...
        def _refresh(*_):
            ...
            for it in items:
                d, r = it["data"], it["row"]
                ok = q in d["name"].lower() or q in d["description"].lower() \
    ...
    def _delete_script(self, path: str, parent: ctk.CTkToplevel) -> None:
        if messagebox.askyesno("Delete Script", f"Delete {os.path.basename(path)}?"):
            try: os.remove(path)
            except OSError as exc: messagebox.showerror("Delete Error", str(exc)); return
            parent.destroy(); self._on_library()
